...
**What:** 限制 Harvester 可抓取的 URL 范围，加入 private IP、localhost、link-local、重定向、请求大小、超时和 allowlist/approval 策略。

**Why:** 第一版只做 API token 和 audit。这个入口边界可以支撑个人手动使用，但 agent 自动 propose source 前必须补抓取边界，否则容易误抓内网管理口、NAS、路由器或 Firecrawl 管理 API。
